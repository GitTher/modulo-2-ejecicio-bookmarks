...
console.log(Array.isArray(bmkData)); */
...
function renderTags(bmkData) {
  let htmlTags = `
    <ul class="item__tags">`;

  if( bmkData[0] !== '' ) {
    htmlTags += `
      <li class="item__tag">${bmkData[0]}</li>`;
  }
  
  if( bmkData[1] !== '' ) {
    htmlTags += `
      <li class="item__tag">${bmkData[1]}</li>`;
  }
...
let html = renderBookmark(bmkData[0]);
html += renderBookmark(bmkData[1]);
html += renderBookmark(bmkData[2]);
